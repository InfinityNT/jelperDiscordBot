...
module.exports = async (client) => {
	...
	const results = await roleSchema.find();
	...
	for (const result of results) {
		...
		if (!guild) {
			console.log(`RoleReaction: Eliminando el servidor: ${guildId} de la base de datos.`);
			await roleSchema.deleteOne({ guildId });
			return;
		}
		...
		if (!channel) {
			console.log(`RoleReaction: Eliminando el canal: ${channelId} de la base de datos.`);
			await roleSchema.deleteOne({ channelId });
			return;
		}
		...
		try {
			...
			console.log(`RoleReaction: Eliminando el mensaje: ${messageId} de la base de datos`);
			await roleSchema.deleteOne({ messageId });
		}
	}
};
